...
#[cfg(feature = "sm-winit")]
use winit::platform::unix::WindowExtUnix as WindowExt;
...
impl Connection {
    ...
    pub fn create_native_widget_from_winit_window(&self, window: &Window)
        ...
        // Default to no scaling if no monitor exist
        let hidpi_factor = *window.primary_monitor().map(|m| m.scale_factor()).get_or_insert(1.0);
        let window_size = window.inner_size().to_logical(hidpi_factor);
